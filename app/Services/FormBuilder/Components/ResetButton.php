...
class ResetButton extends BaseComponent
{
    public function compile($data, $form)
    {
        ...
        if(!$elementName){
            echo 'save form to see submit button';
            return '';
        }
        ...
        if (ArrayHelper::get($data, 'settings.button_style') == '') {
            ...
            if ($activeStates) {
                $styles .= 'form.fluent_form_' . $form->id . ' .wpf_has_custom_css.ff-btn-reset { ' . $activeStates . ' }';
            }
            $hoverStates = '';
            ...
            if ($hoverStates) {
                $styles .= 'form.fluent_form_' . $form->id . ' .wpf_has_custom_css.ff-btn-reset:hover { ' . $hoverStates . ' } ';
            }
        } else if($btnStyle != 'no_style') {
            $styles .= 'form.fluent_form_' . $form->id . ' .ff-btn-reset { background-color: ' . ArrayHelper::get($data, 'settings.background_color') . ' !important; color: ' . ArrayHelper::get($data, 'settings.color') . ' !important; }';
        }

        $atts = $this->buildAttributes($data['attributes']);
        $cls = trim($align . ' ' . $data['settings']['container_class']);

        $html = "<div class='{$cls} ff_reset_btn_wrapper'>";
        ...
        if (isset($data['settings']['button_ui'])) {
            if ($data['settings']['button_ui']['type'] == 'default') {
                ...
                $html .= "<button class='ff-btn-reset' type='reset'><img style='max-width: 200px;' src='{$data['settings']['button_ui']['img_url']}' alt='Reset Form'></button>";
            }
        } else {
